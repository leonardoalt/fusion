...
#[generator = "7"]
pub struct GoldilocksFieldConfig;
pub type GoldilocksField = Fp64<MontBackend<GoldilocksFieldConfig, 1>>;
...
pub fn poseidon_gl(inputs: &[GoldilocksField]) -> [GoldilocksField; 4] {
    ...
    for round in 0..30 {
        ...
        for i in 0..12 {
            state[i] += GoldilocksField::from(ROUND_CONSTANTS[i][round]);
        }
        ...
        for i in 0..12 {
            ...
            for (j, &input) in mds_input.iter().enumerate() {
                tmp += input * GoldilocksField::from(MDS_MATRIX[i][j]);
            }
            state[i] = tmp;
        }
    }
...
pub fn poseidon_gl_u64(inputs: [u64; 12]) -> [u64; 4] {
    let inputs: Vec<_> = inputs.iter().map(|&x| GoldilocksField::from(x)).collect();
    let output = poseidon_gl(&inputs);
    output.map(|x| x.into_bigint().0[0])
}
...
mod tests {
    ...
    fn run_test(inputs: &[u64], expected_output: &[u64]) {
        let inputs: Vec<_> = inputs.iter().map(|&x| GoldilocksField::from(x)).collect();
        let output = poseidon_gl(&inputs);
        let output: [u64; 4] = output.map(|x| x.into_bigint().0[0]);
    ...
    fn test_poseidon_gl() {
        ...
        let minus_one: u64 = 0xffffffff00000001 - 1;
        let inputs: Vec<u64> = vec![minus_one; 12];
        let expected = vec![
        ...
        let inputs: Vec<u64> = vec![18446744069414584321; 12];
        let expected = vec![
            4330397376401421145,
            14124799381142128323,
            8742572140681234676,
